...
[[ "${body}" == *'modifier.state'* ]] || fail "smoke must read WLOC modifier state"
[[ "${body}" == *'10451'* ]] || fail "smoke must check the WLOC interceptor port when active"
[[ "${body}" == *'gs-loc.apple.com'* ]] || fail "smoke must verify WLOC DNS hijack entries when active"
