...
$install = (Get-Content -Path (Join-Path $root "install.sh") -Raw -Encoding UTF8) + ((Get-Content -Path (Join-Path $root "lib/setup-*.sh") -Raw -Encoding UTF8) -join "`n") + (Get-Content -Path (Join-Path $root "lib/host-setup.sh") -Raw -Encoding UTF8)
...
Assert-Contains $install '__MTPROTO_RULE__' 'nft managed template reserves client MTProto slot'
Assert-Contains $install '__CLASH_REMOTE_RULE__' 'nft managed template reserves clash-remote slot'
Assert-Contains $install '5gpn-mtproto' 'MTProto firewall rules are tagged'
Assert-Contains $install '5gpn-clash-remote' 'clash-remote firewall rules are tagged'
