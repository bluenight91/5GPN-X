...
[[ "${api_body}" == *'def run_doctor_json('* ]] || fail "api doctor must use run_doctor_json helper"
[[ "${api_body}" == *'systemd-run'* ]] || fail "api doctor should prefer systemd-run for host probes"
