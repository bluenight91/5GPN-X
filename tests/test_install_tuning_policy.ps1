...
$install = (Get-Content -Path $installPath -Raw -Encoding UTF8) + ((Get-Content -Path (Join-Path $root "lib/setup-*.sh") -Raw -Encoding UTF8) -join "`n") + (Get-Content -Path (Join-Path $root "lib/host-setup.sh") -Raw -Encoding UTF8)
