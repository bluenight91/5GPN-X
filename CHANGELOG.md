...
- WebUI 登录页现在记住控制面板地址和 API 令牌（localStorage 持久化，旧 sessionStorage 会话令牌自动迁移），重新打开页面自动连接；「清除」按钮仍会彻底擦除凭据。注意：令牌持久化在本机浏览器中，公用设备上请务必用「清除」退出。

### Fixed

- doctor 检查多客户端网段时，按 mosdns 的空格分隔格式比对 CIDR，避免配置实际已刷新却因持久化值使用逗号而误报警告。
