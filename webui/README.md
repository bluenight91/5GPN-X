...
2. 确认这个 API 端口(默认 `8444`)能从你浏览器所在的网络访问。5GPN-X 默认不接管主机防火墙(`FIREWALL_MODE=preserve`),`--setup-api` 只会提示你放行该端口,需自行放行;或在安装时用 `FIREWALL_MODE=auto` 让安装器增量放行。
...
打开面板 → 填 **API 地址** `https://你的网关域名:8444` + **令牌**(网关上 `--setup-api` 生成,或 Telegram Bot `/api` 查看)→ 连接。
...
- **面板能开但连不上 API**:检查网关 8444 是否放行、令牌是否正确、网关证书是否有效(面板是 HTTPS,API 也必须是可信 HTTPS)。
...
- 想换令牌:改网关上 `/opt/5gpn/etc/api.env` 里的 `API_TOKEN`,然后 `systemctl restart 5gpn-api`。
- 分享截图/链接给别人时,别把令牌也带上。
